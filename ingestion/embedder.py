...
class EmbeddingGenerator:
    ...
    def __init__(
        ...
        self.model_configs = {
            ...
            # --- OLLAMA/LOCAL CONFIG ---
            # Set all-minilm to its true dimension size (384)
            "all-minilm": {"dimensions": 384, "max_tokens": 512},
            "nomic-embed-text": {"dimensions": 768, "max_tokens": 8192},
        }

        # Check if the model is configured, otherwise use environment fallback
        if model not in self.model_configs:
            logger.warning(f"Unknown model {model}, using VECTOR_DIMENSION from .env")
            # Use 384 as the safe fallback dimension for local models
            self.config = {"dimensions": int(os.getenv("VECTOR_DIMENSION", "384")), "max_tokens": 8191}
        else:
